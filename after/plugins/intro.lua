...
local header_fmt = [[
Type :Tutor and <Enter> for an interactive lesson
]]

local header = vim.split(header_fmt, "\n")
local header_win_width = vim.iter(header):fold(0, function(acc, line)
  return math.max(acc, vim.api.nvim_strwidth(line))
end)
...
EphemeralWidget.new({
  ...
  win_opts = {
    ...
    zindex = 25,
    focusable = false,
    width = header_win_width,
    height = #header,
    anchor = "NW",
    row = 2,
    col = math.floor((WIDTH - header_win_width) / 2),
  },
  highlights = {
    { "^.*$", "LineNr" },
    { ":Tutor", "NvimBlue" },
