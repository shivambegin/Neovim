...
diagnostic.print = function(messages)
  ...
  for _, message in ipairs(messages) do
    ...
    local wrapped =
      diagnostic.wrap(message.message, diagnostic.config.width - diagnostic.len(icon, diagnostic.config.parts.item))
      ...
      if onBottom == true and i == #_o then
        if l == 1 and onRight == true then
          ...
          table.insert(stack, {
            string.rep(
              " ",
              diagnostic.config.width - used_col - (diagnostic.len(item.icon, diagnostic.config.parts_inverse.bottom))
            ),
          })
          table.insert(stack, {
            string.format("%+" .. used_col .. "s", line),
            item.in_range == true and diagnostic.config.active_hl or diagnostic.config.default_hl,
          })
        ...
        elseif l == 1 and onRight == false then
          ...
          table.insert(
            stack,
            { line, item.in_range == true and diagnostic.config.active_hl or diagnostic.config.default_hl }
          )
      ...
      elseif onBottom == false and i == 1 then
        if l == 1 and onRight == true then
          ...
          table.insert(stack, {
            string.rep(
              " ",
              diagnostic.config.width - used_col - (diagnostic.len(item.icon, diagnostic.config.parts_inverse.top))
            ),
          })
          table.insert(stack, {
            string.format("%+" .. used_col .. "s", line),
