...
function M.hl.group(hlgroup, base)
  return vim.tbl_deep_extend(
    'force', base or {}, { fg = hl_prop(hlgroup, 'foreground'), bg = hl_prop(hlgroup, 'background') }
  )
end
...
function M.hl.mode(base)
  ...
  return function()
    ...
    return M.hl.group(
      'Feline' .. M.modes[vim.fn.mode()][2], vim.tbl_deep_extend(
        'force', type(lualine_opts) == 'table' and lualine_opts or { fg = C.bg_1, bg = M.modes[vim.fn.mode()][3] },
        base or {}
      )
    )
  end
...
function M.provider.lsp_progress()
  ...
  return Lsp and string.format(
    ' %%<%s %s %s (%s%%%%) ',
    ((Lsp.percentage or 0) >= 70 and { '', '', '' } or { '', '', '' })[math.floor(vim.loop.hrtime() / 12e7) %
        3 + 1], Lsp.title or '', Lsp.message or '', Lsp.percentage or 0
  ) or ''
end
